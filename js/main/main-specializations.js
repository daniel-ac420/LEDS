...
const specsSection = document.querySelector(".section--specializations");
...
let totalItemsSpec = specsItems.length;
...
//    showLessButtonSpec.style.display = visibleCountSpec > 3 ? 'block' : 'none';
	showLessButtonSpec.style.display = (visibleCountSpec > (window.innerWidth <= 992 ? 3 : 2)) ? 'block' : 'none';
	showLessButtonSpec.style.display = (visibleCountSpec > (window.innerWidth <= 992 ? 3 : 2)) ? 'block' : 'none';
}
...
//    visibleCountSpec = 3;
	if (window.innerWidth <= 992) {
        visibleCountSpec = 3; // Для ширины до 992px
    } else if (window.innerWidth <= 991 && window.innerWidth >= 768) {
        visibleCountSpec = 2; // Для ширины от 991px до 768px
    }
	updateVisibilityParams();
    updateVisibilitySpec();
	
	const headerHeight = document.querySelector("header").offsetHeight;
	window.scrollTo({ top: specsSection.offsetTop - (headerHeight * 1.875), behavior: "smooth" });
});
